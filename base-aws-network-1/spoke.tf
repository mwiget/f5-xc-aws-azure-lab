resource "aws_vpc" "f5-xc-spoke" {
  ...
  tags = {
    Name = "${var.projectPrefix}-f5-xc-spoke-vpc-1"
  }
}
...
resource "aws_subnet" "f5-xc-spoke-external" {
  ...
  tags = {
    Name = "${var.projectPrefix}-f5-xc-spoke-external-${each.key}-1"
  }
}
...
resource "aws_subnet" "f5-xc-spoke-workload" {
  ...
  tags = {
    Name = "${var.projectPrefix}-f5-xc-spoke-workload-${each.key}-1"
  }
}
...
resource "aws_internet_gateway" "f5-xc-spoke-vpc-gw" {
  ...
  tags = {
    Name = "${var.projectPrefix}-f5-xc-spoke-vpc-igw-1"
  }
}
...
resource "aws_route_table" "f5-xc-spoke-vpc-external-rt" {
  ...
  tags = {
    Name = "${var.projectPrefix}-f5-xc-spoke-external-rt-1"
  }
}
...
resource "aws_security_group" "f5-xc-spoke-vpc" {
  name   = "${var.projectPrefix}-f5-xc-spoke-sg-1"
  vpc_id = aws_vpc.f5-xc-spoke.id
